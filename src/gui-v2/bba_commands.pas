...
procedure UploadFileToDCWithBBA;
procedure DownloadFileFromDCWithBBA;

implementation

uses dctool_cfg, tools, main, progress, baudrate, address, utils, upload, download,
     setsize, u_hist, u_progress, bba, u_filters, u_advanced, u_ctrls,
     dl_progress;
...
{  //Test de validité
  ...
  AddBBATreeNode(Upload_Form.Input_Edit.Text, True);
  
  //DCTOOL a utiliser
  if Dctool_Form.External_RadioButton.Checked = True then
    UpCommandLine := Dctool_Form.Location_Edit.Text
  else UpCommandLine := DCTOOLIP; //UpCommandLine := GetTempDir + 'DC-TOOL-IP.EXE';
  ...
  CmdMini := UpCommandLine + '"' + MinimizeName(Upload_Form.Input_Edit.Text,
    Upload_Form.FileName_Label.Canvas, 200) + '" -t ' + ReadIP + ' -a '
      + Address_Form.Address.Text + PutMiniAdvancedCommandLine; //' -b ' + SetBaudrate_Form.Baudrate.Text + ' -a ' + Address_Form.Address.Text;
  ...
  UpCommandLine := UpCommandLine + '"' + Upload_Form.Input_Edit.Text + '" -t '
    + ReadIP + ' -a ' + Address_Form.Address.Text + PutAdvancedCommandLine; //+ ' -b ' + SetBaudrate_Form.Baudrate.Text
...
{  if Download_Form.Output_Edit.Text = '' then
  ...
  if Dctool_Form.External_RadioButton.Checked = True then
    ...
    //DlCommandLine := GetTempDir + 'DC-TOOL-IP.EXE';
    DlCommandLine := DCTOOLIP;
  end;
  ...
  CmdMini := DlCommandLine + '"' + MinimizeName(Download_Form.Output_Edit.Text,
    Download_Form.FileName_Label.Canvas, 200) + '" -t ' + ReadIP + ' -s '
      + SetSize_Form.Size.Text + ' -a ' + Address_Form.Address.Text
        + PutMiniAdvancedCommandLine; //+ ' -b ' + SetBaudrate_Form.Baudrate.Text
  ...
  DlCommandLine := DlCommandLine + '"' + Download_Form.Output_Edit.Text + '" -t '
    + ReadIP + ' -s ' + SetSize_Form.Size.Text + ' -a '
      + Address_Form.Address.Text + PutAdvancedCommandLine;//+ ' -b ' + SetBaudrate_Form.Baudrate.Text
  ...
  AddBBATreeNode(Download_Form.Output_Edit.Text, False);
