...
type
  ...
  private
    ...
    procedure InitMusic;
    procedure FinalizeMusic;
  public
    { Déclarations publiques }
  end;

var
  AboutBox_Form: TAboutBox_Form;
...
procedure TAboutBox_Form.InitScrollingCredits;
...
begin
  with RichView do begin
     ...
     AddTextFromNewLine('Beta testers :', rvsSubheading);
     ...
     AddTextFromNewLine('JMD, L@ Cible, dcprogfr, erwan, diwee, oggy,' +
      ...
      + ' yngwie, Kirkrum, sekk, Rom1, Radigo, GroSeb, everyone...'+ crlf+' ', rvsNormal);
     ...
     + ' Fackue/LyingWake, Curtiss Grymala, Ian Michael, Metafox, Max Scharl, Marcus,'
...
procedure TAboutBox_Form.tScrollingTextTimer(Sender: TObject);
begin
  DoScroll;
end;

procedure TAboutBox_Form.FormCreate(Sender: TObject);
begin
...
procedure TAboutBox_Form.DoScroll;
begin
...
procedure TAboutBox_Form.tRunningSonicTimer(Sender: TObject);
begin
  RunAnim;
end;

procedure TAboutBox_Form.Image5Click(Sender: TObject);
begin
  ShellExecute(Handle, 'open', 'http://www.phidels.com/', '', '', SW_SHOWNORMAL);
end;

procedure TAboutBox_Form.pCloseMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  (Sender as TPanel).BevelOuter := bvLowered;
end;

procedure TAboutBox_Form.pCloseMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  (Sender as TPanel).BevelOuter := bvRaised;
end;

procedure TAboutBox_Form.pCloseMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  (Sender as TPanel).Font.Color := clBlue;
end;

procedure TAboutBox_Form.FormMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TAboutBox_Form.pCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TAboutBox_Form.RunButtonsAnim;
begin
  with pClose do
    while Left <> 496 do
    begin
...
procedure TAboutBox_Form.InitButtonsAnim;
begin
  pClose.Left := 620;
  pSiZ.Top := 600;
end;

procedure TAboutBox_Form.Image6Click(Sender: TObject);
begin
  ShellExecute(Handle, 'open', 'http://dctop.free.fr/in.php?id=57', '', '', SW_SHOWNORMAL);
end;

procedure TAboutBox_Form.FinalizeMusic;
begin
  XMFree;
end;

procedure TAboutBox_Form.InitMusic;
begin
  XMLoadFromRes('SPACE', 'MUSIC');
  XMPlay;
end;

procedure TAboutBox_Form.FormDestroy(Sender: TObject);
begin
  FinalizeMusic;
end;
